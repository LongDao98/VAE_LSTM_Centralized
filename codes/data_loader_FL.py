...
class DataGenerator(BaseDataGenerator):
  ...
  def load_NAB_dataset(self, dataset, y_scale=6):
    ...
    n_train_sample1 = int(n_train_sample * 0.4)
    n_train_sample2 = n_train_sample - n_train_sample1
    data1 = data['training'][:n_train_sample1]
    data2 = data['training'][n_train_sample1:]
    ...
    for k in range(self.config['l_win']):
      n_not_overlap_wins = (n_train_sample1 - k) // self.config['l_win']
      ...
      for i in range(n_train_lstm):
        ...
        for j in range(self.config['l_seq']):
          # print(k,i,j)
          cur_seq[j] = data1[k + self.config['l_win'] * (j + i): k + self.config['l_win'] * (j + i + 1)]
    ...
    n_train_lstm1 = lstm_seq.shape[0]
    idx_train1, idx_val1, self.n_train_lstm1, self.n_val_lstm1 = self.separate_train_and_val_set(n_train_lstm1)
    self.train_set_lstm1 = dict(data=np.expand_dims(lstm_seq[idx_train1], -1))
    self.val_set_lstm1 = dict(data=np.expand_dims(lstm_seq[idx_val1], -1))

    #train and var set 2
    for k in range(self.config['l_win']):
      n_not_overlap_wins = (n_train_sample2 - k) // self.config['l_win']
      n_train_lstm = n_not_overlap_wins - self.config['l_seq'] + 1
      cur_lstm_seq = np.zeros((n_train_lstm, self.config['l_seq'], self.config['l_win']))
      for i in range(n_train_lstm):
        cur_seq = np.zeros((self.config['l_seq'], self.config['l_win']))
        for j in range(self.config['l_seq']):
          # print(k,i,j)
          cur_seq[j] = data2[k + self.config['l_win'] * (j + i): k + self.config['l_win'] * (j + i + 1)]
        cur_lstm_seq[i] = cur_seq
      if k == 0:
        lstm_seq2 = cur_lstm_seq
      else:
        lstm_seq2 = np.concatenate((lstm_seq2, cur_lstm_seq), axis=0)

    n_train_lstm2 = lstm_seq2.shape[0]
    idx_train2, idx_val2, self.n_train_lstm2, self.n_val_lstm2 = self.separate_train_and_val_set(n_train_lstm2)
    self.train_set_lstm2 = dict(data=np.expand_dims(lstm_seq2[idx_train2], -1))
    self.val_set_lstm2 = dict(data=np.expand_dims(lstm_seq2[idx_val2], -1))
