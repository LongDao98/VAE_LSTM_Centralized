...
class DataGenerator(BaseDataGenerator):
  ...
  def load_NAB_dataset(self, dataset, y_scale=6):
    ...
    rolling_windows = np.zeros((n_train_vae, self.config['l_win'], self.config['n_channel'])) #them n_channel
    ...
    for k in range(self.config['l_win']):
      ...
      cur_lstm_seq = np.zeros((n_train_lstm, self.config['l_seq'], self.config['l_win'], self.config['n_channel'])) #them n_channel
      for i in range(n_train_lstm):
        cur_seq = np.zeros((self.config['l_seq'], self.config['l_win'], self.config['n_channel'])) #them n_channel
